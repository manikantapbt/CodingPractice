public class SubTree {
    public static void main(String[] args) {
        Nodesh newRoot = new Nodesh(80);
        Nodesh root = new Nodesh(20);
        newRoot.left = root;
        newRoot.right = new Nodesh(50);
        ...
        System.out.println(subTree(newRoot, root1));
    }

    public static boolean subTree(Nodesh root, Nodesh root1) {
        if (root == null)
            return false;
        if (root1 == null)
            return true;
        boolean areIdentical = identicalTrees(root, root1);
        if (areIdentical)
            return true;
        else
